...
*         ; the list remains sorted by part number. Prints  *
*         an error message and returns prematurely if the   *
*         part already exists or space could not be         *
...
/************************************************************
* erase: Prompts the user to enter a part number, if it     *
*        exists, deletes the part (frees it from memory);   *
...
void erase(void)
{
	...
	if (find_part(number) != NULL)
	{
		...
		printf("Part name: %s\n", cur->name);
		printf("Are you sure you want to erase the part?");
		printf(" (y- yes/ n- no): ");
		for (;;)
		{
			scanf(" %c", &validation);
			...
			if (validation == 'y')
			{
				...
				free(cur);
				printf("Part removed from database.\n");
				return;
			}
