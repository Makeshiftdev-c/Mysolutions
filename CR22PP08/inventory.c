...
/***********************************************************
 * dump: Prompts the user to enter an output file name.    *
 *       If the file can be created/opened, saves the      *
 *       database contents in the file; otherwise, prints  *
 *       an error message and returns prematurely.         *
 *       Indicates succesful write operation; otherwise,   *
 *       prints error message and returns prematurely.     *
 ***********************************************************/
...
void dump(void)
{
	...
	putc(num_parts, fp_out);
	if (fwrite(inventory, sizeof(struct part), num_parts, fp_out) < num_parts)
	{
		fprintf(stderr, "Error encountered during write operation.\n");
		fclose(fp_out);
		return;
	}
...
/***********************************************************
 * restore: Prompts the user to enter name of file,        *
 *          containing previously saved database content.  *
 *          If the file exists and contents are valid,     *
 *          indicates successful read operation; otherwise,*
...
void restore(void)
{
	...
	parts_saved = getc(fp_in);
	if (fread(inventory, sizeof(struct part), parts_saved, fp_in) < parts_saved)
	{
		fprintf(stderr, "Error encountered during read operation.\n");
		fclose(fp_in);
		return;
	}
	printf("Read operation successful.\n");
	num_parts = parts_saved;
	fclose(fp_in);
}
